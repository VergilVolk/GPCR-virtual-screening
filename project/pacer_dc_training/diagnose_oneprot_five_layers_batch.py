...
parser.add_argument(
    "--replica", type=int, choices=[2, 3], required=True
)
parser.add_argument(
    "--window", type=int, choices=range(5), required=True
)
parser.add_argument(
    "--context",
    choices=[
...
UNIT = ROOT / f"replica_{args.replica:02d}" / f"window_{args.window:03d}"
...
OUT = ROOT / f"five_layer_diagnostic_R{args.replica}_W{args.window}_v01"
...
assert summary["replica_id"] == args.replica
assert summary["window_id"] == args.window
assert summary["start_frame"] == args.window * 100
assert summary["completeness"] == "complete"

atom_path = UNIT / "atom14" / f"{CONTEXT}_w{args.window:03d}.npy"
csv_path = atom_path.with_suffix(".csv")
history_path = UNIT / "embeddings" / f"{CONTEXT}_w{args.window:03d}.npy"
...
for name, tensor in captured.items():
    np.save(
        OUT / f"{name}_{CONTEXT}_R{args.replica}_W{args.window}.npy",
        tensor.numpy()[0],
    )
...
(OUT / f"five_layer_report_{CONTEXT}.json").write_text(
...
print("G1_2F_EXPORT_PASS:", all(
    (OUT / f"{name}_{CONTEXT}_R{args.replica}_W{args.window}.npy").is_file()
